...
rows = re.findall(r"^\| ([^|]+) \| \[([^\]]+)\]\(([^)]+)/解读\.md\) \| (.+?) \| \[(?:PDF|官方来源)\]\(([^)]+)\)( ⚠️)? \| (\d+) \|$", s, re.M)
...
for date, short, d, title, url, warn, cn in rows:
    ...
    r = chk.get(d, {})
    errs = []
    if not os.path.exists(f"{d}/解读.md"): errs.append("解读.md 不存在")
    ...
    body = io.open(f"{d}/解读.md", encoding="utf-8").read()
    if len(re.findall(r"[一-鿿]", body)) != int(cn): errs.append("字数不符")
    seen.add(d)
    if errs:
        bad += 1; print(f"  !! {d}: {'; '.join(errs)}")
allq = {d for d in os.listdir(".") if os.path.isdir(d) and d[0].isdigit()}
