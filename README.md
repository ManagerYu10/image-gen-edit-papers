...
> 都写在 [可信度与产出.md](docs/可信度与产出.md) 里，引用之前请先看一眼。
...
拉到本页最后的[全量清单](#全量清单)，186 行按时间排，浏览器里 `Ctrl/Cmd + F` 搜简称或标题。每行长这样：
...
第一批 160 篇是固定的 11 节，每节写什么、写多长都由 [prompt.md](docs/prompt.md) 规定，机器校验齐全才算通过：

| 节 | 写什么 | 你什么时候看它 |
| --- | --- | --- |
| 1. 30 秒定位 | 一句话说清这篇解决什么 | 判断这篇要不要读 |
| 2. 为什么这篇论文会出现 | 前作卡在哪 | 补脉络 |
| 3. 读懂前需要的最小概念 | 术语和前置知识 | 不熟这个子方向时先看 |
| 4. 方法全景：先看数据流 | ASCII 数据流图 + 组件拆解 | 想知道它到底怎么跑 |
| 5. 核心公式与直觉 | 1～4 个必需公式，逐个解释变量 | 要理解为什么这么设计 |
| 6. 训练到底怎么跑 | 一轮训练 step 的完整顺序 | 要复现 |
| 7. 推理到底怎么跑 | 完整推理顺序 | 要接入线上 |
| 8. 实验和消融到底证明了什么 | ≥4 条关键证据的表格 + 哪些主张还没被证明 | **要拿数字做决策** |
| 9. 与前作怎么选 | 和 2～4 个最接近方案比，何时选何时不选 | 做选型 |
| 10. 工程判断：可行性、风险与上限 | 数据、算力、实现难点、失败环节、监控信号 | **要评估落地代价** |
| 11. 放回研究脉络并收束 | 5 件必须记住的事 + 5 道自测题 | 读完自查 |

另外 26 项是 2026-08-29 补的决策型短笔记，8～9 节，不追字数；其中 4 项没有独立论文（FLUX.1、Grok/Aurora、Qwen-Image-Edit-2511、GPT Image 2），只写官方资料和核验边界。
两批的差别见 [可信度与产出.md §1](docs/可信度与产出.md#1-两批笔记格式和验收口径都不一样)。

### 3. 不知道该从哪篇开始

| 你要干的事 | 进这里 |
| --- | --- |
| 系统补齐五年脉络 | [INDEX.md](docs/INDEX.md) §1——15 条技术脉络分组，每篇一句话说清它解决什么 |
| 只要跨论文的结论，不想一篇篇读 | [总结分析.md](docs/总结分析.md)——五年的真正转折点、15 条脉络怎么互相咬合、全领域还没解决的六条 |
| 先看一眼五年主线长什么样 | [主线.html](docs/主线.html)，clone 后用浏览器打开（GitHub 网页上只会显示源码） |
| 判断某篇是不是真发表了、被引多少 | 先查 `meta.json` 的 `publication.venue`，没有就查 [`docs/review/`](docs/review/) 下的四份 manifest |
| 要拿库里的数字去做决策 | 先读 [可信度与产出.md](docs/可信度与产出.md)——做过哪些校验、哪些地方已知会出错 |

### 4. 要原文 PDF

⚠️ **PDF 不在仓库里**，182 份合计 3.3 GB，超出 GitHub 建议的仓库体积。单篇点表里的 `PDF` 链接就行；要批量拉到本地：

```bash
python3 scripts/fetch_pdfs.py                    # 补齐所有缺的，182 份 / 3.3 GB / 约 10 分钟
python3 scripts/fetch_pdfs.py --list             # 只列缺什么，不下载
python3 scripts/fetch_pdfs.py 2024-11_OmniEdit   # 只下一篇
```

只用标准库，不需要装依赖。按 arXiv 要求节流到每 3 秒一份，断了重跑只补没下成的。
...
186 项按时间排。**简称**点开是中文解读，**原文**点开是论文本身——182 条标 `PDF` 的都过了四项核验（HTTP 206、`Content-Type: application/pdf`、前 4 字节是 `%PDF`、**远端总字节数与本地 `paper.pdf` 完全一致**），
4 项标 `官方来源` 的确实没有独立论文。口径和结果见 [可信度与产出.md §2](docs/可信度与产出.md#2-原文直链是怎么核的)。
